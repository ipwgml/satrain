...
def test_load_baseline_results(base_sensor):
    ...
    assert f"GPROF V07 ({base_sensor.upper()})" in results.algorithm
    ...
    assert f"GPROF V07 ({base_sensor.upper()})" in results_austria.algorithm

    results = load_baseline_results(base_sensor, baselines=["era5"])
    assert "ERA5" in results.algorithm
    assert f"GPROF V07 ({base_sensor.upper()})" not in results.algorithm
